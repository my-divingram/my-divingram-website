...
export const getStaticPaths = async() => {
    // 200件以上の場合は要対策
    const data = await client.get({ endpoint: "uwphoto", queries: { filters: `class[equals]damselfish`, limit: 100 }});
    const data_ = await client.get({ endpoint: "uwphoto", queries: { filters: `class[equals]damselfish`, limit: 100, offset: data.totalCount-100 }});
...
export default function IndividualPage({pagedata}){
    ...
    return (
        <Layout title={title} description={description} url={url} imageUrl={pagedata.thumbImg.url}>
            <Species classes="スズメダイの仲間" pagedata={pagedata}></Species>
        </Layout>
    )
}