...
export const getStaticPaths = async() => {
    const data = await client.get({ endpoint: "uwphoto", queries: { filters: `class[equals]angelfish`, limit: 100 }});
...
export default function IndividualPage({pagedata}){
    ...
    const description = `${pagedata.japaneseName}の生態写真`
    const url = `https://my-divingram-website.vercel.app/fish/angelfish/${pagedata.latinName}`.replace(" ", "_")

    return (
        <Layout title={title} description={description} url={url} imageUrl={pagedata.thumbImg.url}>
            <Species classes="キンチャクダイの仲間" pagedata={pagedata}></Species>
        </Layout>
    )
}