...
export const getStaticPaths = async() => {
    const data = await client.get({ endpoint: "uwphoto", queries: { filters: `class[equals]butterflyfish`, limit: 100 }});
...
export default function IndividualPage({pagedata}){
    ...
    return (
        <Layout title={pagedata.japaneseName}>
            <Species classes="チョウチョウウオの仲間" pagedata={pagedata}></Species>
        </Layout>
    )
}